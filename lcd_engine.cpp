...
// I'm taking off the top map (#3) - so I can use it for scoring etc.
// This takes off 8 pixels from the height - so it's now 56
const uint8_t pagemap[] = { 2, 1, 0, 7, 6, 5, 4 };
...
void lcd_engine::drawbitmap(uint8_t x, uint8_t y, 
...
void lcd_engine::drawstring(uint8_t x, uint8_t line, char *c) {
  while (c[0] != 0) {
    drawchar(x, line, c[0]);
    c++;
    x += 6; // 6 pixels wide
...
void lcd_engine::draw_text_string(uint8_t x, uint8_t line, String s) {
    int start, i;
    char c;
    i = s.length();
    start = 0;
    while (start != i) {
      c = s.charAt(start);
      draw_engine_char(x, line, c);
      start ++ ;
      x += 6;
    }
}

// Ok - changing this only to write to the special text screen
void lcd_engine::draw_engine_string(uint8_t x, uint8_t line, char *c) {
  while (c[0] != 0) {
    draw_engine_char(x, line, c[0]);
    c++;
    x += 6; // 6 pixels wide
    if (x + 6 >= LCDWIDTH) {
      x = 0;    // ran out of this line
...
void  lcd_engine::draw_engine_char(uint8_t x, uint8_t line, char c) {
  for (uint8_t i = 0; i<5; i++) {
    text_buffer[x+(line*128)] = pgm_read_byte(font+(c*5)+i);
    x++;
  }
}


void  lcd_engine::drawchar(uint8_t x, uint8_t line, char c) {
  for (uint8_t i =0; i<5; i++ ) {
...
void lcd_engine::drawline(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, 
		      uint8_t color) {
...
// filled rectangle
void lcd_engine::fillrect(uint8_t x, uint8_t y, uint8_t w, uint8_t h, 
		      uint8_t color) {
...
// draw a rectangle
void lcd_engine::drawrect(uint8_t x, uint8_t y, uint8_t w, uint8_t h, 
		      uint8_t color) {
  // stupidest version - just pixels - but fast with internal buffer!
  for (uint8_t i=x; i<x+w; i++) {
    setpixel(i, y, color);
...
void lcd_engine::drawcircle(uint8_t x0, uint8_t y0, uint8_t r, 
			uint8_t color) {
  int8_t f = 1 - r;
  int8_t ddF_x = 1;
  int8_t ddF_y = -2 * r;
...
void lcd_engine::fillcircle(uint8_t x0, uint8_t y0, uint8_t r, 
			uint8_t color) {
  int8_t f = 1 - r;
  int8_t ddF_x = 1;
  int8_t ddF_y = -2 * r;
...
void lcd_engine::setpixel(uint8_t x, uint8_t y, uint8_t color) {
...
void lcd_engine::st7565_init(void) {
  // set pin directions
  pinMode(sid, OUTPUT);
  pinMode(sclk, OUTPUT);
  pinMode(a0, OUTPUT);
...
inline void lcd_engine::spiwrite(uint8_t c) {
  shiftOut(sid, sclk, MSBFIRST, c);
 

}
void lcd_engine::st7565_command(uint8_t c) {
  digitalWrite(a0, LOW);

  spiwrite(c);
}

void lcd_engine::st7565_data(uint8_t c) {
  digitalWrite(a0, HIGH);

  spiwrite(c);
}

void lcd_engine::st7565_set_brightness(uint8_t val) {
    st7565_command(CMD_SET_VOLUME_FIRST);
    st7565_command(CMD_SET_VOLUME_SECOND | (val & 0x3f));
}


void lcd_engine::display(void) {
  uint8_t c, p;

	// changing this to 7 - to reflect the page change
  for(p = 0; p < 7; p++) {
    /*
      putstring("new page! ");
      uart_putw_dec(p);
...
void lcd_engine::engine_display(void) {
  uint8_t c, p;

	// only one page - the first one
    st7565_command(CMD_SET_PAGE | 3); // 3or 1??
    st7565_command(CMD_SET_COLUMN_LOWER | (0x0 & 0xf));
    st7565_command(CMD_SET_COLUMN_UPPER | ((0x0 >> 4) & 0xf));
    st7565_command(CMD_RMW);
    st7565_data(0xff);
    ...
    for(c = 0; c < 128; c++) {
      st7565_data(text_buffer[(128*p)+c]);
    }
  }


// clear everything
void lcd_engine::clear(void) {
  memset(buffer, 0, 1024);
}


// this doesnt touch the buffer, just clears the display RAM - might be handy
void lcd_engine::clear_display(void) {
  uint8_t p, c;
  // same here
  for(p = 0; p < 7; p++) {
...
// Clear the engine special text space

void lcd_engine::clear_text_display(void) {
  uint8_t p, c;
  // Just need to clear the first space
  st7565_command(CMD_SET_PAGE | 3 ); // 3 is the page that I took off
  for (c=0; c < 129; c++ ) {
  st7565_command(CMD_SET_COLUMN_LOWER | (c & 0xf));
  st7565_command(CMD_SET_COLUMN_UPPER | ((c >> 4) & 0xf));
  st7565_data(0x0);
